SUBROUTINE READ_OBSERVED_DATANEW(N,NS,dobs,fname)
    USE global_variables, ONLY: generic_dir_name, casename,cwd
    IMPLICIT NONE
    CHARACTER*400                                   :: FILENAME,fname
    INTEGER, INTENT(IN)                             :: N      ! number of observed data
    INTEGER, INTENT(IN)                             :: NS     ! number of time steps
    REAL*8, DIMENSION(NS,N+2),INTENT(INOUT)         :: DOBS
    CHARACTER*80                                    :: A1
    INTEGER                                         :: i,k,l,f,N2,j,nskip
    LOGICAL                                         :: FEXIST
    double precision                                :: tmp
    nskip = 9
    FILENAME=trim(cwd) // '\'//trim(fname)
        ...
        IF (FEXIST) THEN
            ...
            !outer: do
                ...
                elseif (N+2>13) then
                    ...
                    do j=1,NS
                        read(1,*) (dobs(j,i),i=1,13)
                    end do
                    N2 = N
                    N2 = N2 - 11
                    l = 13
1002                If(N2+1>13) then
                        do i=1,nskip
                            read(1,*) A1
                        end do
                        do j=1,NS
                            read(1,*) tmp,(dobs(j,i),i=l+1,l+12)
                        end do
                        N2 = N2 - 12
                        l  = l + 12
                        goto 1002
                        ...
                        do j=1,NS
                            read(1,*) tmp,(dobs(j,i),i=l+1,l+N2)
                        end do
                        !exit outer
                    end if
                end if
