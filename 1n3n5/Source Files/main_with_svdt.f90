...
!        
!        DO k=1,Nd*Nta
!            Cf_DD (k,k) = Cf_DD(k,k) + alpha_i*s_d(k)**2
!        END DO
!        
!        call inv_svdt(Cf_DD,Nd*Nta,1.0d0)
!        Cf_MD = MATMUL(Cf_MD,Cf_DD)
!        !RHO_md(1:,1:) = Cf_MD(1:,1:)
!        !call bootstrap(N_e,Np*N,Nd*Nta,20,Cf_MD,m_uc(1:Np*N,1:N_e),df(1:Nd*Nta,1:N_e),s_d, alpha_i)
!        
!        !open(1,file='.\Ke_m.txt',status='unknown')
!        !do i=1,Np*N
!        !    write(1,'(12E20.8)') (Cf_MD(i,j),j=1,Nd*Nta)
!        !end do
!        !close(1)
!        !open(1,file='.\Ke_local_m.txt',status='unknown')
!        !do i=1,Np*N
!        !    write(1,'(12E20.8)') (RHO_md(i,j),j=1,Nd*Nta)
!        !end do
!        !close(1)
!
!201     err_par = 0.0d0
